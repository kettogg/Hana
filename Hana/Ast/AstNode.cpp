...
namespace hana
{
	...
	Value *String::codeGen(CodeGenContext &context)
	{
		...
		ArrayType *ArrayTy_0 = ArrayType::get(
			IntegerType::get(context.getGlobalContext(), 8), value.size() + 1);
		...
		Constant *const_array_str =
			ConstantDataArray::getString(context.getGlobalContext(), value);
		...
		ConstantInt *const_int = ConstantInt::get(context.getGlobalContext(),
	...
	Value *Identifier::codeGen(CodeGenContext &context)
	{
		...
		Node::printError(location,
						 "** [Err] Undeclared variable" + structName + " :: " + name);
		context.addError();
		return nullptr;
	}
