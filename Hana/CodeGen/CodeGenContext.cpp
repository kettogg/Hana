...
namespace hana {
    ...
    CodeGenContext::CodeGenContext(std::ostream &outs) : outs(outs) {
        ...
        module = new llvm::Module("hana", llvmContext);
    }
...
#define MAKE_LLVM_EXTERNAL_NAME(a) #a
    ...
    void CodeGenContext::setupBuiltIns() {
        ...
        f = Function::Create(ft, Function::ExternalLinkage, MAKE_LLVM_EXTERNAL_NAME(writeflt), getModule());
        i = f->arg_begin();
        if (i != f->arg_end())
            i->setName("val");
        builtins.push_back({f, (void *) writeflt});
        ...
        f = Function::Create(ft, Function::ExternalLinkage, MAKE_LLVM_EXTERNAL_NAME(readflt), getModule());
        builtins.push_back({f, (double *) readflt});
    ...
    GenericValue CodeGenContext::runCode() {
        outs << ":: Running code...\n\n";
        std::string err;
        ExecutionEngine *ee = EngineBuilder(std::unique_ptr<Module>(module)).setErrorStr(&err).setEngineKind(
    ...
    void
    CodeGenContext::klassAddVariableAccess(std::string name, int index) { classAttributes[klassName][name] = index; }
    ...
    Instruction *CodeGenContext::getKlassVarAccessInst(std::string klass, std::string name, AllocaInst *this_ptr) {
        ...
        if (this_ptr->getType()->getElementType()->isPointerTy()) {
            ...
            return GetElementPtrInst::Create(val->getType()->getPointerElementType(), val, ptr_indices, "",
                                             currentBlock());
        }
        Instruction *ptr = GetElementPtrInst::Create(this_ptr->getType()->getElementType(), this_ptr, ptr_indices, "",
                                                     currentBlock());
        return ptr;
    }
    ...
    std::string CodeGenContext::findClassNameByType(llvm::Type *ty) {
        auto found = std::find_if(std::begin(classTypeMap), std::end(classTypeMap),
                                  [&](auto kv) { return kv.second == ty; });
